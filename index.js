...
const config = {
    ...
    clientID: 'rOOLwMgSXuW3w3laRd0folpHqhtjQnmV',
    issuerBaseURL: 'https://dev-tj2q2qbq8rq327er.us.auth0.com'
  };
...
app.post("/publish",(req,res)=>{
    db.collection('Publish').countDocuments({user_name:req.body.user_name},(err,obj)=>{
        if (err) throw err;
        if(obj==0){
            ...
            res.send("<script>alert('Published Ride Successfully'); window.location.href = '/home';</script>");
        }
        else{
            db.collection('Publish').updateOne({user_name:req.body.user_name}, {$set:req.body}, function(err, res) {
                if (err) throw err;
                db.collection('AllPublishedRides').insertOne(req.body,(err,collection)=>{
                    if (err) throw err;
                    console.log("Record inserted successfully");
...
app.post("/riderdetails", function (req, res) {
    db.collection('Publish').deleteOne({user_name:req.body.rider_name},(err,obj)=>{
...
app.post("/bookride", function (req, res) { 
    ...
    res.send("<script>alert('Ride Booked Successfully.'); window.location.href = '/home';</script>");
})
...
app.post("/viewdetails", function (req, res) { 
    ...
    db.collection('AllPublishedRides').find({user_name:uname}).toArray(function(err, docs) {
        if(JSON.stringify(docs) === '[]'){
...
app.post("/adminloginform", function (req, res) {  
    db.collection('AdminUsers').countDocuments({username:req.body.username,password:req.body.password},(err,obj)=>{
        if (err) throw err
        else if(obj==1){
            res.send("<script>alert('You are an Admin.'); window.location.href = '/ridedetailspage';</script>");
        }
        else{
...
app.post("/loginform", function (req, res) {  
    db.collection('RegisterUsers').countDocuments({username:req.body.username,password:req.body.password},(err,obj)=>{
        if (err) throw err
        else if(obj==1){
            res.send("<script>alert('You have Login Successfully.'); window.location.href = '/home';</script>");
        }
        else{
            res.send("<script>alert('Please Register First'); window.location.href = '/registerpage';</script>");
        }
    })
})
...
app.post("/registerform", function (req, res) {
    db.collection('RegisterUsers').countDocuments({username:req.body.username},(err,obj)=>{ 
        if (err) throw err;
        else if(obj==1){
            res.send("<script>alert('Email Exits.Please Login'); window.location.href = '/loginpage';</script>");
        }
        else{
            db.collection('RegisterUsers').insertOne(req.body,(err,collection)=>{
                if(err){
                    throw err;
                }
                console.log("Registered Record Inserted Successfully");
            });
            res.send("<script>alert('You have registered successfully.'); window.location.href = '/home';</script>");
        }
    })
})
